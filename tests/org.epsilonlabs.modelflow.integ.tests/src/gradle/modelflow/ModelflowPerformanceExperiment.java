...
public class ModelflowPerformanceExperiment extends IterativeExperiment {
	...
	protected void beforeTestExecution() throws Exception {
		...
		/** Module Workflow */
		module = spy(new ModelFlowModule());
		Workflow workflow = EcoreUtil.copy(getComponentWorkflow());
		when(module.getWorkflow()).thenReturn(workflow);
		when(module.getTaskFactoryRegistry()).thenReturn(taskFactoryRegistry);
		when(module.getResFactoryRegistry()).thenReturn(resFactoryRegistry);
	...
	private static final Path ROOT = Paths.get(System.getProperty("user.home"),"SW","PhD","experiments","2020-MODELS-ModelFlowGradle","out","gradle.workflow");
	...
	public static Workflow getComponentWorkflow() {
		...
		Path sourceDirectory = ROOT.resolve(Paths.get("..","..","gradle.workflow")).normalize();
