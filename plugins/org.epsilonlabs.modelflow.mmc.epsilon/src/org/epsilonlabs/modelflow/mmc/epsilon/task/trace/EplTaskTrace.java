...
public class EplTaskTrace implements ITrace {
	...
	public EplTaskTrace(PatternMatch match, ITask task) {
		this(match, task, null);
	}
	
	private EplTaskTrace(PatternMatch match, ITask task, Trace trace) {
	...
	public ITrace init() {
		...
		patternMatch.getRoleBindings().forEach((k,v) -> {
			AbstractResource model = EpsilonTraceUtil.getContainerModel(task, v);
